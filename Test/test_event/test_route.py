...
class TestEventRoute(unittest.TestCase):
    ...
    def test_event_get_page(self):
        ...
        self.assertEqual(data[names.ANSWER], names.SUCCESS)
        test_dict_answer = {names.NAME: 'Test_event', 'description': 'test', 'status': '1', 'date_start': '04.12.2017', 'date_end': '04.12.2017'}
        self.assertEqual(data[names.DATA][0], test_dict_answer)
    ...
    def test_event_get_all(self):
        data = json.loads(rq.get('{url}/event'.format(url=URL)).text)
        self.assertEqual(data[names.ANSWER], names.SUCCESS)
        #self.assertEqual(data[names.DATA], dict())
